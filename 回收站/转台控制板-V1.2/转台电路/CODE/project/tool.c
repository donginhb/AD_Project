...
/****************************************************
	函数名:	TimeTask_SetState
	功能:	设置任务状态
	参数:	任务ID	任务状态
	返回值:	无
	说明:	
	作者:	liyao 2015年11月29日15:08:39
****************************************************/
void TimeTask_SetState(int8_t TaskID,TASK_STATE state){
	TimeTaskList[TaskID].taskState = state;
}
...
void TimeTask_Run(void){
	...
	while(curTT != NULL){
		...
		if(lapse >= curTT->interval && curTT->taskState == NORMAL){ //时间差大于等于任务间隔则执行
